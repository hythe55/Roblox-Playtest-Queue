...
TOOLS = [
 {"name":"acquire","description":"Unless the user explicitly asks to bypass or skip the queue for this action, call `acquire` immediately before any blocking Roblox Studio action. Blocking actions include playtests and MCP edits to edit-time instances. Filesystem edits to scripts do not require the queue. Await `acquire` before proceeding; it returns only when the lease is ready. Do not report queue details while waiting. Use the same stable `agent` and `job_id` for `release`. If a call times out, retry with the same values. The lease lasts 900 seconds.","inputSchema":{"type":"object","properties":{"agent":{"type":"string"},"job_id":{"type":"string"}},"required":["agent","job_id"]}},
...
def handle(msg):
    ...
    try:
        if method == "initialize": reply(i,{"protocolVersion":"2024-11-05","capabilities":{"tools":{}},"serverInfo":{"name":"roblox-playtest-queue","version":"0.2.0"},"instructions":"Unless the user explicitly asks to bypass or skip the queue for a playtest, immediately before calling acquire clearly tell the user that you are requesting a position in the Roblox Studio playtest queue, then call acquire before any Roblox Studio playtest. If the user explicitly asks to bypass or skip the queue, do not call acquire. Acquire waits internally and returns only when ready; do not report queue details while waiting. Use the same agent and job_id for release."})
